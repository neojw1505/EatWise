          ...
          <div class="cardStyle p-2 rounded-4">
            ...
            >
              ...
              <div class="d-flex justify-content-between my-3">
                <div>{{ i.name }}</div>
                <div>{{ i.measures.metric.amount }} {{ i.measures.metric.unitShort }}</div>
              </div>
        ...
        <div class="col-lg-6">
          ...
          <div class="cardStyle p-4  rounded-4">
            ...
            <div class="d-flex flex-wrap">
              <div class="bg-light text-black p-2 m-1 rounded-2 border border-warning" v-for="diet in recipeDetails.diets" :key="diet">{{ diet }}</div>
            </div>
          </div>
          <!-- give brief nutrients -->
          <div class="cardStyle p-4  rounded-4">
            <div class="fw-semibold mb-3 ">Nutrition:</div>
            <div class="d-flex flex-wrap text-center">
              <div v-for="nutrient of showEssentialNutrients" :key="nutrient" class="rounded-4 p-1 m-1" style="background-color: #FBE8A6; width: 100px;">
                <div class="text-dark bg-light rounded-5 p-2">{{ nutrient.amount.toFixed(0) }}{{ nutrient.unit }}</div>
          ...
          <div class="cardStyle p-2 rounded-4" v-if="recipeDetails.instructions">
            ...
            >
            <div class="mx-2">
              <div class="text-black bg-light justify-content-center align-items-center d-flex " 
...
export default {
  ...
  computed: {
    formattedRecipeSummary() {
      let temp=this.recipeDetails.summary.replace(/<a[^>]*>(.*?)<\/a>/g, '<a  font-weight:bold;">$1</a>').split(".");
      let res=[]
...
{name: 'Carbs', amount: eachNutrient.amount, unit: eachNutrient.unit, percentOfDailyNeeds: eachNutrient.percentOfDailyNeeds})
continue
          }
