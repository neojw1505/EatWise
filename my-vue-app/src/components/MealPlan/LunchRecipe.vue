<template>
  <div class="card shadow border-light mx-auto" style="overflow: auto; min-width: 150px; max-width: 500px;" v-if="recipeData && nutritionData">
    <div class="card-header"><h3 style="font-size: 2.5vw;">Lunch</h3></div>
  ...
  <div class="card shadow text-center" v-else>
    ...
    <div class="card-footer border-0">
      ...
      <button class="btn btn-lg btn-fail rounded-4" @click="toggleConsumedState()" :class="{ 'consumed-btn-green': isConsumed }">
  ...
  methods: {
    ...
    async toggleConsumedState(date) {
      if (this.isConsumed) {
        ...
        Toast.fire({
          icon: "error",
          title: `You did not eat <b style='color: red;'>${this.recipeData.title}</b> for Lunch!`,
        });
      } else {
        this.isConsumed = !this.isConsumed;
        ...
        Toast.fire({
          icon: "success",
          title: `You ate <b style='color: green'>${this.recipeData.title}</b> for Lunch!`,
        });
      }
    },
    ...
    async SetEatenLunchinFB() {
      await this.$smAPI.SetEatenLunchinFB(
        this.recipeData,
        this.nutritionData,
        this.currentDate,
        this.currentTime
