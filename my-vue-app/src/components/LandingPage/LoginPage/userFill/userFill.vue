...
export default {
  ...
  mounted() {
    ...
    earWiggle.to(".ear-left", { duration: 0.1, rotation: 0 });
    ...
    const xMovement = (innerEyeWidth - pupilWidth) / 2; // Adjust the divisor to control the movement
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      // Calculate the position of the pupils to follow the cursor
      const posX =
        (mouseX / window.innerWidth) * (innerEyeWidth - pupilWidth) - xMovement;
      const posY =
        (mouseY / window.innerHeight) * (innerEyeHeight - pupilHeight) -
        yMovement; // Adjust the subtraction here

      // Apply the transformation to the pupils
